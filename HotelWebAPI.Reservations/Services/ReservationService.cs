...
using HotelWebAPI.Payments.Entities;
...
namespace HotelWebAPI.Main.Services
{
    public class ReservationService
    {
        ...
        public async Task<Tuple<bool, string>> ConfirmReservation(int id)
        {
            ...
            var paymentResult = await _paymentResolver.ResolveFor<AddPaymentDto>(newPayment, "hotelPayments/payment/add");
            ...
            var invoiceResult = await _paymentResolver.ResolveFor<AddInvoiceDto>(newInvoice, "hotelPayments/invoice/add");
        ...
        public async Task<Tuple<bool, string>> CancelReservation(AddRefundDto dto)
        {
            ...
            if (reservation.Confirmed)
            {
                var refundResult = await _paymentResolver.ResolveFor<AddRefundDto>(dto, "hotelPayments/refund/add");
